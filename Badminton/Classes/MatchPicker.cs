    ...
    public class MatchPicker
    {
        public void PickMatch(string? GameType, Session session, BadmintonClub badmintonClub)
        {
            ...
            if (session.MatchPreview.Team1Players.Count == 0)
            {
                ...
                for (b = 0; b < session.WaitingPlayers.Count || session.MatchPreview.Team1Players.Count > 1; b++)
                {
                    for (c = 0; c < session.WaitingPlayers.Count || session.MatchPreview.Team2Players.Count > 0; c++)
                    {
                        for (d = 0; d < session.WaitingPlayers.Count || session.MatchPreview.Team2Players.Count > 1; d++)
                        {
                ...
                if (session.MatchPreview.Team1Players.Count < 2)
                {
                    ...
                    session.WaitingPlayers.Remove(session.WaitingPlayers[pickedPlayers[1]]);
                }
                if (session.MatchPreview.Team2Players.Count < 1)
                {
                    session.MatchPreview.Team2Players.Add(session.WaitingPlayers[pickedPlayers[2]]);
                    session.WaitingPlayers.Remove(session.WaitingPlayers[pickedPlayers[2]]);
                }
                if (session.MatchPreview.Team2Players.Count < 2)
                {
                    session.MatchPreview.Team2Players.Add(session.WaitingPlayers[pickedPlayers[3]]);
                    session.WaitingPlayers.Remove(session.WaitingPlayers[pickedPlayers[3]]);
                }
